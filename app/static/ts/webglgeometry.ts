...
export function createGrid(xmax: number, ymax: number, step: number, color: Array<number>) {
    const vertices = [];
    color[0] /= 255;
    color[1] /= 255;
    color[2] /= 255;
...
function createNode(radius: number, latitudeBands: number, longitudeBands: number): Float32Array {
    const vertices = [];
    ...
    for (let lat = 0; lat < latitudeBands; ++lat) {
        for (let lon = 0; lon <= longitudeBands; ++lon) {
            ...
            vertices.push(
                ...vertices.slice(current*7, current*7 + 7),
                ...vertices.slice(next*7, next*7 + 7),
                ...vertices.slice((current + 1)*7, (current + 1)*7 + 7),

                ...vertices.slice((current + 1)*7, (current + 1)*7 + 7),
                ...vertices.slice(next*7, next*7 + 7),
                ...vertices.slice((next + 1)*7, (next + 1)*7 + 7)
            );
        }
    }
...
function createLine(vertices: Array<number>, color: Array<number>): Float32Array {
    const line = [];
    color[0] /= 255;
    color[1] /= 255;
    color[2] /= 255;
...
function createShadowPlane(size: number): Float32Array {
    const vertices = [];
...
function createStaticBuffer(gl: WebGL2RenderingContext, data: ArrayBuffer) {
...
function createVAO(gl: WebGL2RenderingContext, program: WebGLProgram, data: ArrayBuffer, attrs: Map<string, [number, number]>) {
...
type UniformFunc = (program: WebGLProgram, data: Float32Array) => void;
...
export class Shape {
    ...
    setViewProjection(vp: mat4) {
        this.vp = vp;
    }
    draw(
        vp: mat4,
        program: WebGLProgram,
        attrs: Map<string, [number, number]>,
        uniforms: Map<keyof Shape, UniformFunc>
    ) {
        this.vp = vp;
