...
import {Shape} from "./webglgeometry.js";

type UniformFunc = (program: WebGLProgram, data: Float32Array) => void;

function createShader(gl: WebGL2RenderingContext, src: string, type: number): WebGLShader {
...
function setFrameBufferSize(gl: WebGL2RenderingContext, tex: WebGLTexture, buff: WebGLRenderbuffer, width: number, height: number) {
...
export class Program {
    ...
    protected vp = new mat4();
    protected col: [number, number, number, number];
    public x = 0;
    public y = 0;
    public z = 0;
    ...
    constructor (
        ...
        protected attrs: Map<string, [number, number]>,
        protected uniforms: Map<keyof Shape, UniformFunc>,
        vs: string,
        fs: string,
    ) {
...
export class Picker extends Program {
    ...
    draw() {
        ...
        const x = this.x*this.gl.canvas.width/(this.gl.canvas as HTMLCanvasElement).clientWidth;
        const y = this.gl.canvas.height - this.y*this.gl.canvas.height/(this.gl.canvas as HTMLCanvasElement).clientHeight -1;
